...
class App_OverviewClass(TKMT.ThemedTKinterFrame):
    ...
    def create_exam_overview(self):
        """
        Prints list of all exams for that class for that year/term
        :return:
        """

        self.notebook.nextCol()
        self.label_frame = self.addLabelFrame("asdf")
        self.label_frame.Label("some text")

    def create_categories_overview(self):
        """
    ...
    def create_student_overview(self):
        ...
        for tab in self.tabs:
            tab.nextCol()
            tab.treeview = tab.Treeview(["Nachname", "Vorname"], [120, 120], 10, students, 'subfiles', ['lastname', 'firstname'])
            tab.treeview.bind("<<TreeviewSelect>>", self.on_click_student)
